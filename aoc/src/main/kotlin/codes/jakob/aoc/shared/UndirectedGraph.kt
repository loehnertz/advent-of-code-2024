...
class UndirectedGraph<T>(input: List<Pair<T, T>>) {
    ...
    private val vertices: Set<Vertex<T>> = edges.flatMap { setOf(it.a, it.b) }.toSet()
    ...
    fun findBestPath(
        ...
        fun reconstructPath(last: Vertex<T>): List<Vertex<T>> {
        ...
        while (open.isNotEmpty()) {
            ...
            if (current == to) return reconstructPath(current)
